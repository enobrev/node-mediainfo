...
var MediaInfo = function(sFileName, fCallback) {
    ...
    this.get = function(sFileName) {
        ...
        var oMediaInfo = exec(command, function(oError, oStdOut, oStdErr) {
            if (oError !== null) {
                ...
                if (iCode !== 0) {
                    this.onError(iCode, new Error(aStdError));
                } else if (sOut.length) {
                    this.parse(sOut);
                } else if (bOutput) {
                    this.onError(0, new Error('No Output'));
                } else {
                    this.onError(0, new Error('No Output At All'));
                }   
            }
...
/*        
        ...
        oMediainfo.on('close', function () {
            if (iCode !== 0) {
                this.onError(iCode, new Error(sError));
            } else if (sOut.length) {
                this.parse(sOut);
            } else if (bOutput) {
                this.onError(0, new Error('No Output'));
