...
var MediaInfo = function(sFileName, sCmdPath, fCallback) {
    this.fCallback = typeof fCallback == 'function' ? fCallback : function() {};
    this.oInfo     = {};
    this.cmdPath   = sCmdPath || 'mediainfo';
    ...
    this.get = function(sFileName) {
        ...
        var iCode       = 0;
        var bOutput     = false;
        var command     = [this.cmdPath, '--Full', '--Output=XML','"'+sFileName+'"'].join(' ');
        var oMediaInfo  = exec(command, {maxBuffer: 1024 * 1024 * 100}, function(oError, oStdOut, oStdErr) {  // 5 Mb Buffer since it may have a cover art image
            if (oError !== null) {
                iCode = oError.code;
